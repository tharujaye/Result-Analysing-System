...
def histogram_display(c1, c2, c3, c4):
    ...
    message1 = Text(Point(400, 50), "Histogram Results")
    ...
    Text(Point(400, 570), f"{total_students} Outcome(s) in total.").draw(win)
...
while True:
    if credit_list == []:
        ...
        if replay == 'y':
            ...
            print("\nPart 2:")
            for output, credits in zip(output_list, credit_list):
                print(f"{output} - {credits[0]}, {credits[1]}, {credits[2]}")
            break
